export interface CloneApiConfig {
  port: number;
  /** Default: `file:./portrait.db` (local libSQL). Override with `postgres://…` for enterprise. */
  databaseUrl: string;
  ...
  /** Display name when loading a portrait from SQLite by `portraitId` (optional). */
  portraitSubjectName: string | null;
  vectorTopK: number;
  rerankTopN: number;
...
export function loadConfig(overrides: Partial<CloneApiConfig> = {}): CloneApiConfig {
  return {
    ...
    databaseUrl:
      overrides.databaseUrl ?? env("DATABASE_URL") ?? "file:./portrait.db",
    ...
    portraitSubjectName:
      overrides.portraitSubjectName ?? env("PORTRAIT_SUBJECT_NAME") ?? null,
